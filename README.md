...
- win：alt+m 
- mac：shift+tab
普通模式，claude code每次创建文件或者是修改文件的时候，都必须经过用户审阅批准。
![](./cla.asstes/mo1.png)
...
! bash模式下，允许执行临时终端命令行命令（例如`npm install`），无需额外开启窗口，直接执行bash命令。
![](./cla.asstes/tan.png)
...
### 一图了解Skills、MCP、Sub-agents角色定位
具体阅读：[Claude Skills： 从指令到资产的系统化构建指南](https://www.axtonliu.ai/newsletters/ai-2/posts/claude-agent-skills-maps-framework)

![](./cla.asstes/skill10.png)
...
### Plan mode模式
将模式切换到Plan mode模式下，对话框输入`在/plan mode文件中生成热搜新闻页面，使用react + vite + shadcn/ui，ui风格简约美观即可`。
claude code会帮你规划页面的生成计划，如下图：
![](./cla.asstes/plan1.png)

1.清除上下文，同时进入同意模式，后续修改文件就不再询问用户
2.进入同意模式，后续修改文件就不再询问用户
3.进入默认模式，每次创建文件或者是修改文件的时候，都必须经过用户审阅批准。
4.继续修改计划，claude code根据要求再次修改生成新的计划
![](./cla.asstes/plan2.png)
我们选择第一项，进入同意模式
![](./cla.asstes/plan3.png)

经过几分中的等待，完成热搜新闻的页面生成。
![](./cla.asstes/plan4.png)
按crlt+b将这个npm run任务进行后台运行，通过/tasks查看后台运行的任务
![](./cla.asstes/plan5.png)
按`k`结束task任务
![](./cla.asstes/plan6.png)

如果需要回滚对话框输入`/rewind`，比如我现将当前页面添加一个鼠标移动阴影功能，下图是claude code添加一个新的功能，我需要将此功能去掉进行回滚。
![](./cla.asstes/plan7.png)
对话框输入`/rewind`，选择要回滚的对话记录
![](./cla.asstes/plan8.png)
四个选项，我选择回滚代码和会话即可滚回到之前代码
1.回滚代码和会话
2.只回滚会话
3.只回滚代码
4.从此处汇总
5.放弃回滚
![](./cla.asstes/plan9.png)
### 小记
...
```shell
$ cloude --dangerously-skip-permissions
或
